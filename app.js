...
(function () {
  ...
  function zastosujZnaleziska(lista) {
    ...
    for (const z of stan.znaleziska) {
      if (z.status !== 'otwarte' && z.status !== 'odrzucone') continue;
      let przesuniecie = 0;
      let kolizja = false;
  ...
  function elementZnaleziska(z) {
    ...
    if (z.status !== 'otwarte') {
      ...
      st.textContent = z.status === 'zaakceptowane' ? '✓ zastosowano' :
        (z.status === 'odrzucone' ? 'odrzucono' : 'nieaktualne');
      nag.appendChild(st);
    }
    div.appendChild(nag);
    ...
    ctx.addEventListener('click', () => odswiezPodglad(z.status === 'otwarte' || z.status === 'odrzucone' ? z : null));
    div.appendChild(ctx);
  ...
  $('btn-pobierz-raport').addEventListener('click', () => {
    ...
    w.push('## Statystyki');
    w.push('');
    w.push('| Kategoria | Znaleziska | Zastosowane | Odrzucone |');
    w.push('|---|---|---|---|');
    const wgKat = {};
    ...
    for (const kat of Object.keys(OO.KATEGORIE)) {
      ...
      w.push(`| ${kat}. ${OO.KATEGORIE[kat]} | ${g.length} | ${zast} | ${odrz} |`);
    }
    w.push('');
    const LIMIT = 1500;
    for (const kat of Object.keys(OO.KATEGORIE)) {
